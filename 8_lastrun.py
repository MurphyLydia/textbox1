...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\29817\\Documents\\2\\4\\8_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    opening = visual.TextStim(win=win, name='opening',
    ...
    response_instr = visual.TextStim(win=win, name='response_instr',
        text='请输入下一个词:',
        font='Arial',
        pos=(-0.208, -0.185), draggable=True, height=0.044, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=-2.0);
    underline = visual.Line(
        ...
        size=(0.3, 0),
        ori=0.0, pos=(0.12, -0.222), draggable=False, anchor='center',
        lineWidth=1.0,
        colorSpace='rgb', lineColor='white', fillColor='white',
        opacity=None, depth=-3.0, interpolate=True)
    response_textbox = visual.TextBox2(
         win, text=None, placeholder=None, font='Arial',
         ori=0.0, pos=(0.1, -0.185), draggable=False,      letterHeight=0.05,
         size=(0.2604, 0.0556), borderWidth=2.0,
    ...
    end_instr = visual.TextStim(win=win, name='end_instr',
        ...
        font='Arial',
        pos=(0, -0.185), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=-5.0);
    ...
    underline_hint = visual.Line(
        ...
        size=(0.3, 0),
        ori=0.0, pos=(-0.83, 0.37), draggable=False, anchor='center',
        lineWidth=1.0,
        colorSpace='rgb', lineColor='white', fillColor='white',
    ...
    while continueRoutine:
        ...
        # if opening is starting this frame...
        if opening.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            opening.frameNStart = frameN  # exact frame index
            opening.tStart = t  # local t and not account for scr refresh
            opening.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(opening, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'opening.started')
            # update status
            opening.status = STARTED
            opening.setAutoDraw(True)
        
        # if opening is active this frame...
        if opening.status == STARTED:
            # update params
            pass
    ...
    SentenceLoop = data.TrialHandler2(
        name='SentenceLoop',
        nReps=1.0, 
        method='random', 
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('sentences.xlsx'), 
        seed=None, 
        isTrials=True, 
    )
    ...
    if thisSentenceLoop != None:
        for paramName in thisSentenceLoop:
            globals()[paramName] = thisSentenceLoop[paramName]
    if thisSession is not None:
        # if running in a Session with a Liaison client, send data up to now
        thisSession.sendExperimentData()
    
    for thisSentenceLoop in SentenceLoop:
        SentenceLoop.status = STARTED
        if hasattr(thisSentenceLoop, 'status'):
            thisSentenceLoop.status = STARTED
        ...
        if thisWordLoop != None:
            for paramName in thisWordLoop:
                globals()[paramName] = thisWordLoop[paramName]
        if thisSession is not None:
            # if running in a Session with a Liaison client, send data up to now
            thisSession.sendExperimentData()
        
        for thisWordLoop in WordLoop:
            WordLoop.status = STARTED
            if hasattr(thisWordLoop, 'status'):
                thisWordLoop.status = STARTED
            ...
            Loop = data.Routine(
                name='Loop',
                components=[shown_stim, response_instr, underline, response_textbox, end_instr, tmp, underline_hint],
            )
            ...
            while continueRoutine:
                ...
                if hasattr(thisWordLoop, 'status') and thisWordLoop.status == STOPPING:
                    continueRoutine = False
                # get current time
                t = routineTimer.getTime()
                tThisFlip = win.getFutureFlipTime(clock=routineTimer)
                ...
                if cur_word_idx < len(words)-1 :
                    ...
                    y_line = 0.33
                    underline_hint = visual.Rect(
                        win,
                        width = remaining_width, height = 0.004,
                        pos=(x_center, y_line),
                        fillColor='white'
                    )
                    underline_hint.draw()
            ...
            endClock.reset()
            WordLoop.addData('response_textbox.text',response_textbox.text)
            # the Routine "Loop" was not non-slip safe, so reset the non-slip timer
            routineTimer.reset()
            # mark thisWordLoop as finished
            if hasattr(thisWordLoop, 'status'):
                thisWordLoop.status = FINISHED
            # if awaiting a pause, pause now
            if WordLoop.status == PAUSED:
        ...
        # completed 1.0 repeats of 'WordLoop'
        WordLoop.status = FINISHED
        
        if thisSession is not None:
            # if running in a Session with a Liaison client, send data up to now
            thisSession.sendExperimentData()
        # mark thisSentenceLoop as finished
        if hasattr(thisSentenceLoop, 'status'):
            thisSentenceLoop.status = FINISHED
        # if awaiting a pause, pause now
        if SentenceLoop.status == PAUSED:
            thisExp.status = PAUSED
            pauseExperiment(
                thisExp=thisExp, 
                win=win, 
